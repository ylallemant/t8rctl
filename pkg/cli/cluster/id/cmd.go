...
import (
	...
	"github.com/ylallemant/t8rctl/pkg/cli/cluster/id/options"
...
var rootCmd = &cobra.Command{
	Use:   "id",
	Short: "returns the active id for the specified cluster group and datatier",
	Long:  ``,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(filteredClusters) == 0 {
			return fmt.Errorf("no cluster was found with filter: %s => %s-%s", provider.Type(), options.Current.Group, options.Current.Datatier)
		}

		if len(filteredClusters) > 1 {
			return fmt.Errorf("filter returned multiple clusters (%d): %s => %s-%s", len(filteredClusters), provider.Type(), options.Current.Group, options.Current.Datatier)
		}

		fmt.Println(filteredClusters[0].Id())
